...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(225, 100),  # All numbers are approximate.
                rg.Point(150, 175),
                rg.Point(75, 100),
                rg.Point(150, 25)]
...
def run_test_draw_points_on_circle():
    ...
    title = 'Test 4:  80 purple dots on black circle.'
    window = rg.RoseWindow(400, 400, title)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    point = generate_points_on_circle(circle, number_of_slices)
    for k in range(number_of_slices):
        line = rg.Line(circle.center, point[k])
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(160, 100), 100)
    circle.outline_thickness = 3
    polygon(window, circle, 4, 'blue', 5)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        line = rg.Line(p[k%len(p)], p[(k+1)%len(p)])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(200, 200), 100)
    fancy_polygon(window, circle, 20, 9, 'navy', 3)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    j = generate_points_on_circle(circle, number_of_lines)
    for k in range(number_of_lines):
        line = rg.Line(j[k%len(j)], j[(k+hops_to_next_point)%len(j)])
        line.color = color
        line.arrow = 'last'
        line.thickness = thickness
        line.attach_to(window)
